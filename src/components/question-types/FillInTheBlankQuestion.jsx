...
export function FillInTheBlankQuestion({ 
  ...
  const fetchFillInTheBlankQuestion = async () => {
    setLoading(true);
    setError(null);
    
    try {
      const response = await fetch(`http://localhost:8080/api/quizzes/random?type=fill-in-blank&level=${formData.level}`);
      ...
      console.error('빈칸 채우기 문제 로드 실패:', err);
        ...
        <CardContent className="!p-8">
          <div className="flex items-center justify-center !py-12">
            <div className="text-center">
              ...
              <p className="text-gray-600">빈칸 채우기 문제를 준비하고 있어요...</p>
            </div>
          </div>
        </CardContent>
      </Card>
      ...
      <Card className="w-full max-w-4xl !px-4 !py-8">
        <CardContent className="!p-8">
          <div className="text-center !py-12">
            ...
            <button 
              onClick={fetchFillInTheBlankQuestion}
  ...
  const renderQuestionWithBlank = (questionText) => {
    ...
    return questionText.split('____').map((part, index, array) => (
      <React.Fragment key={index}>
        {part}
        {index < array.length - 1 && (
          <span className="bg-yellow-200 px-2 py-1 rounded font-bold text-blue-800">
            ____
          </span>
        )}
      </React.Fragment>
    ...
    <Card className="w-full max-w-4xl !px-4 !py-8">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Badge variant="outline" className="bg-orange-50 text-orange-700 border-orange-200">
              ✏️ {isStudyMode ? '빈칸 채우기' : currentQuestion.category}
            </Badge>
            {studyType && (
      ...
      <CardContent className="!space-y-4">
        ...
        <div className="bg-orange-50 !p-4 rounded-lg border border-orange-200">
          <p className="text-sm text-orange-700 font-medium !mb-2">✏️ 빈칸 채우기</p>
          <p className="text-orange-800">
            문장의 빈칸에 들어갈 가장 적절한 단어를 선택하세요.
          </p>
        </div>
