...
export function Word({ 
  ...
  const fetchWordQuestion = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://localhost:8081/api/quizzes/random?type=word&level=${formData.level}`);
        ...
        <CardContent className="!p-8">
          <div className="flex items-center justify-center !py-12">
            <div className="text-center">
              ...
              <p className="text-gray-600">단어 문제를 준비하고 있어요...</p>
            </div>
          </div>
        </CardContent>
      </Card>
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            <Badge variant="outline" className="bg-green-50 text-green-700 border-green-200">
              💭 {isStudyMode ? '단어 학습' : currentQuestion.category}
            </Badge>
            {studyType && (
